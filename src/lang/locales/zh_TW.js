/* Gogo Language Texts

Table of Contents

01.General
02.User Login, Logout, Register
03.Menu
04.Dashboards
05.Pages
06.Applications
  06.01.Chat
  06.02.Survey
  06.03.Todo
07.UI
  07.01.Alerts
  07.02.Badges
  07.03.Buttons
  07.04.Cards
  07.05.Carousel
  07.06.Charts
  07.07.Collapse
  07.08.Dropdowns
  07.09.Editors
  07.10.Forms
  07.11.Form Components
  07.12.Icons
  07.13.Input Groups
  07.14.Jumbotron
  07.15.Modal
  07.16.Navigation
  07.17.Popover & Tooltip
  07.18.Sortable
  07.19.Maps
  07.20.Tables
  07.21.Wizards
*/

module.exports = {
  /* 01.General */
  'general.copyright': 'Gogo React © 2018 All Rights Reserved.',

  'unauthorized.title': 'Unauthorized Access Attempt',
  'unauthorized.detail':
    'You are not authorized to view the page you are trying to access.',

  /* 02.User Login, Logout, Register */
  'user.login-title': 'Login',
  'user.register': 'Register',
  'user.forgot-password': 'Forgot Password',
  'user.email': 'E-mail',
  'user.password': 'Password',
  'user.forgot-password-question': 'Forget password?',
  'user.fullname': 'Full Name',
  'user.login-button': 'LOGIN',
  'user.register-button': 'REGISTER',
  'user.reset-password-button': 'RESET',
  'user.buy': 'BUY',
  'user.username': 'Username',
  'user.new-password-again': 'New Password Again',
  'user.new-password': 'New Password',
  'user.reset-password': 'Reset Password',
  /* 03.Menu */
  'menu.home': '主頁',
  'menu.app': '主頁',
  'menu.dashboards': 'Dashboards',
  'menu.staffs': '員工',
  'menu.clients': '客戶概覽',
  'menu.admin': 'Admin',
  'menu.help': 'Help',
  'menu.default': 'Default',
  'menu.analytics': 'Analytics',
  'menu.ecommerce': 'Ecommerce',
  'menu.content': 'Content',
  'menu.pages': 'Pages',
  'menu.clients-list': '客戶目錄',
  'menu.data-list': 'Data List',
  'menu.thumb-list': 'Thumb List',
  'menu.image-list': 'Image List',
  'menu.details': 'Details',
  'menu.search': '搜尋',
  'menu.login': 'Login',
  'menu.register': 'Register',
  'menu.forgot-password': 'Forgot Password',
  'menu.error': 'Error',
  'menu.applications': 'Applications',
  'menu.todo': 'To-do List',
  'menu.survey': 'Survey',
  'menu.chat': 'Chat',
  'menu.ui': 'UI',
  'menu.alerts': 'Alerts',
  'menu.badges': 'Badges',
  'menu.buttons': 'Buttons',
  'menu.cards': '智能卡',
  'menu.carousel': 'Carousel',
  'menu.charts': 'Charts',
  'menu.collapse': 'Collapse',
  'menu.dropdowns': 'Dropdowns',
  'menu.editors': 'Editors',
  'menu.form-layouts': 'Form Layouts',
  'menu.form-components': 'Form Components',
  'menu.form-validations': 'Form Validations',
  'menu.form-wizard': 'Form Wizard',
  'menu.icons': 'Icons',
  'menu.input-groups': 'Input Groups',
  'menu.jumbotron': 'Jumbotron',
  'menu.modal': 'Modal',
  'menu.navigation': 'Navigation',
  'menu.popover-tooltip': 'Popover & Tooltip',
  'menu.sortable': 'Sortable',
  'menu.tables': 'Tables',
  'menu.menu': 'Menu',
  'menu.subhidden': 'Subhidden',
  'menu.hidden': 'Hidden',
  'menu.visible': 'Visible',
  'menu.maps': 'Maps',
  'menu.landingpage': 'Landing Page',
  'menu.multipage-home': 'Multipage Home',
  'menu.singlepage-home': 'Singlepage Home',
  'menu.about': 'About',
  'menu.auth-login': 'Auth Login',
  'menu.auth-register': 'Auth Register',
  'menu.reset-password': 'Reset Password',
  'menu.blog': 'Blog',
  'menu.blog-list': 'Blog List',
  'menu.blog-detail': 'Blog Detail',
  'menu.careers': 'Careers',
  'menu.confirmation': 'Confirmation',
  'menu.contact': 'Contact',
  'menu.docs': 'Docs',
  'menu.features': 'Features',
  'menu.prices': 'Prices',
  'menu.videos': 'Videos',
  'menu.mailing': 'Mailing',
  'menu.invoice': 'Invoice',
  'menu.blank-page': 'Blank Page',
  'menu.types': 'Menu Types',
  'menu.levels': 'Menu Levels',
  'menu.third-level-1': 'Third Level 1',
  'menu.third-level-2': 'Third Level 2',
  'menu.third-level-3': 'Third Level 3',
  'menu.faq': 'Faq',
  'menu.knowledge-base': 'Knowledge Base',
  'menu.authorization': 'Authorization',
  'menu.profile': 'Profile',
  'menu.product': 'Product',
  'menu.miscellaneous': 'Miscellaneous',
  'menu.portfolio': 'Portfolio',
  'menu.social': 'Social',
  'menu.details-alt': 'Details Alt',
  'menu.forms': 'Forms',
  'menu.components': 'Components',
  'menu.layouts': 'Layouts',
  'menu.validations': 'Validations',
  'menu.wizard': 'Wizard',

  /* 04.Dashboards */
  'dashboards.pending-orders': 'Pending Orders',
  'dashboards.completed-orders': 'Completed Orders',
  'dashboards.refund-requests': 'Refund Requests',
  'dashboards.new-comments': 'New Comments',
  'dashboards.sales': 'Sales',
  'dashboards.orders': 'Orders',
  'dashboards.refunds': 'Refunds',
  'dashboards.recent-orders': 'Recent Orders',
  'dashboards.product-categories': 'Product Categories',
  'dashboards.cakes': 'Cakes',
  'dashboards.tickets': 'Tickets',
  'dashboards.calendar': 'Calendar',
  'dashboards.best-sellers': 'Best Sellers',
  'dashboards.website-visits': '下載聯繫人資料',
  'dashboards.unique-visitors': '發生率',
  'dashboards.this-week': 'This Week',
  'dashboards.last-week': 'Lat Week',
  'dashboards.this-month': 'This Month',
  'dashboards.conversion-rates': 'Conversion Rates',
  'dashboards.per-session': 'Per Session',
  'dashboards.profile-status': 'Profile Status',
  'dashboards.payment-status': 'Payment Status',
  'dashboards.work-progress': 'Work Progress',
  'dashboards.tasks-completed': 'Tasks Completed',
  'dashboards.payments-done': 'Payments Done',
  'dashboards.order-stock': 'Order - Stock',
  'dashboards.categories': 'Categories',
  'dashboards.quick-post': 'Quick Post',
  'dashboards.title': 'Title',
  'dashboards.content': 'Content',
  'dashboards.category': 'Category',
  'dashboards.save-and-publish': 'Save and Publish',
  'dashboards.top-viewed-posts': 'Top Viewed Posts',
  'dashboards.posts': 'Posts',
  'dashboards.pending-for-publish': 'Pending for publish',
  'dashboards.users': 'Users',
  'dashboards.on-approval-process': 'On approval process',
  'dashboards.alerts': 'Alerts',
  'dashboards.waiting-for-notice': 'Waiting for notice',
  'dashboards.files': 'Files',
  'dashboards.pending-for-print': 'Pending for print',
  'dashboards.logs': '變更記錄',
  'dashboards.gogo': 'GOGO',
  'dashboards.magic-is-in-the-details': 'MAGIC IS IN THE DETAILS',
  'dashboards.yes-it-is-indeed': 'Yes, it is indeed!',
  'dashboards.advanced-search': 'Advanced Search',
  'dashboards.toppings': 'Toppings',
  'dashboards.type': 'Type',
  'dashboards.keyword': 'Keyword',
  'dashboards.search': '搜尋',
  'dashboards.top-rated-items': 'Top Rated Items',


  /* 05.Clients */
  'clients.company-name': '公司名稱',
  'clients.company-code': '公司代碼',
  'clients.company-no_of_license': '員工數量',
  'clients.company-no_of_admin': '管理員數量',
  'clients.company-id': 'Id',
  'clients.company-updatedAt': 'updatedAt',
  'clients.status': '狀態',


  /* 05.Pages */
  'pages.add-new': '新增',
  'pages.add-new-modal-title': 'Add New Item',
  'pages.display-options': 'Display Options',
  'pages.orderby': '按順序 : ',
  'pages.total': '總計: ',
  'pages.viewing': '查看 : ',
  'pages.product-name': 'Product Name',
  'pages.category': 'Category',
  'pages.description': 'Description',
  'pages.status': '狀態',
  'pages.cancel': 'Cancel',
  'pages.submit': '確認',
  'pages.delete': 'Delete',
  'pages.another-action': 'Another action',
  'pages.actions': 'ACTIONS',
  'pages.header': 'Header',
  'pages.details': 'DETAILS',
  'pages.orders': 'ORDERS',
  'pages.rating': 'Rating',
  'pages.price': 'Price',
  'pages.ingredients': 'Ingredients',
  'pages.is-vegan': 'Is Vegan',
  'pages.order-status': 'Order Status',
  'pages.bake-progress': 'Bake Progress',
  'pages.popularity': 'Popularity',
  'pages.comments': 'Comments',
  'pages.error-title': 'Ooops... looks like an error occurred!',
  'pages.error-code': 'Error code',
  'pages.go-back-home': 'GO BACK HOME',
  'pages.mailing-info':
    'Mailing templates use inline styling and table layout to be displayed well at various service providers. To provide better usability we include it as regular html with dangerouslySetInnerHTML.',
  'pages.invoice-info':
    'Invoice template has an inline styled version for usage outside of the project as well as React version.',
  'pages.react-version': 'React Version',
  'pages.inline-version': 'Inline Styled Html Version',
  'pages.like': 'Like',
  'pages.likes': 'Likes',
  'pages.details-title': 'Details',
  'pages.comments-title': 'Comments',
  'pages.questions-title': 'Questions',
  'pages.similar-projects': 'Similar Projects',
  'pages.send': 'Send',
  'pages.addComment': 'Add a comment',
  'pages.location': 'Location',
  'pages.responsibilities': 'Responsibilities',
  'pages.photos': 'Photos',
  'pages.who-to-follow': 'Who to Follow',
  'pages.follow': 'FOLLOW',
  'pages.followers': 'FOLLOWERS',
  'pages.recent-posts': 'Recent Posts',
  'pages.profile': 'PROFILE',
  'pages.friends': 'FRIENDS',
  'pages.images': 'IMAGES',
  'pages.purchase': 'PURCHASE',
  'pages.price.developer': 'DEVELOPER',
  'pages.price.team': 'TEAM',
  'pages.price.enterprise': 'ENTERPRISE',
  'pages.price.twofactorauthentication': 'Two factor authentication',
  'pages.price.teampermissions': 'Team permissions',
  'pages.price.245Support': '24/5 Support',
  'pages.price.247Support': '24/7 Support',
  'pages.price.useractionsauditlog': 'User actions audit log',
  'pages.prices.featurecomparison': 'Feature Comparison',
  'pages.prices.pricecomparison': 'Price Comparison',

  /* 06.Applications */

  /* 06.01.Chat */
  'chat.messages': 'Messages',
  'chat.contacts': 'Contacts',
  'chat.saysomething': 'Say something..',

  /* 06.02.Survey */
  'survey.delete': 'Delete',
  'survey.edit': 'Edit',
  'survey.add-new': '新增',
  'survey.add-new-title': 'Add New Survey',
  'survey.title': 'Title',
  'survey.category': 'Category',
  'survey.label': 'Label',
  'survey.status': '狀態',
  'survey.cancel': 'Cancel',
  'survey.submit': '確認',
  'survey.another-action': 'Another action',
  'survey.display-options': 'Display Options',
  'survey.orderby': '按順序 : ',
  'survey.all-surveys': 'All Surveys',
  'survey.completed-surveys': 'Completed Surveys',
  'survey.categories': 'Categories',
  'survey.active-surveys': 'Active Surveys',
  'survey.labels': 'Labels',

  /* 06.03.Todo */
  'todo.add-new': '新增',
  'todo.add-new-title': 'Add New Todo',
  'todo.title': 'Title',
  'todo.detail': 'Detail',
  'todo.category': 'Category',
  'todo.label': 'Label',
  'todo.status': '狀態',
  'todo.cancel': 'Cancel',
  'todo.submit': '確認',
  'todo.action': 'Action',
  'todo.another-action': 'Another action',
  'todo.display-options': 'Display Options',
  'todo.orderby': '按順序 : ',
  'todo.all-tasks': 'All Tasks',
  'todo.pending-tasks': 'Pending Tasks',
  'todo.completed-tasks': 'Completed Tasks',
  'todo.categories': 'Categories',
  'todo.labels': 'Labels',

  /* 07.UI */

  /* 07.01.Alerts */
  'alert.rounded': 'Rounded Alert',
  'alert.react-notifications': 'React Notifications',
  'alert.outline': 'Outline',
  'alert.primary': 'Primary',
  'alert.secondary': 'Secondary',
  'alert.info': 'Info',
  'alert.success': 'Success',
  'alert.warning': 'Warning',
  'alert.error': 'Error',
  'alert.filled': 'Filled',
  'alert.primary-text': 'This is a primary alert—check it out!',
  'alert.secondary-text': 'This is a secondary alert—check it out!',
  'alert.success-text': 'This is a success alert—check it out!',
  'alert.danger-text': 'This is a danger alert—check it out!',
  'alert.warning-text': 'This is a warning alert—check it out!',
  'alert.info-text': 'This is a info alert—check it out!',
  'alert.light-text': 'This is a light alert—check it out!',
  'alert.dark-text': 'This is a dark alert—check it out!',
  'alert.default': 'Default Alert',
  'alert.dismissing': 'Dismissing Alert',
  'alert.dismissing-text':
    'Holy guacamole! You should check in on some of those fields below.',
  'alert.dismissing-without-animate-text':
    'I am an alert and I can be dismissed without animating!',

  /* 07.02.Badges */
  'badge.sizes': 'Sizes',
  'badge.colors': 'Colors',
  'badge.outline': 'Outline',
  'badge.links': 'Links',
  'badge.counter-badges': 'Counter Badges',
  'badge.bootstrap-default': 'Bootstrap Default',
  'badge.primary': 'Primary',
  'badge.secondary': 'Secondary',
  'badge.success': 'Success',
  'badge.danger': 'Danger',
  'badge.warning': 'Warning',
  'badge.info': 'Info',
  'badge.light': 'Light',
  'badge.dark': 'Dark',

  /* 07.03.Buttons */
  'button.default': 'Bootstrap Default',
  'button.colors': 'Colors',
  'button.rounded': 'Rounded',
  'button.outline': 'Outline',
  'button.states': 'States',
  'button.sizes': 'Sizes',
  'button.button-groups': 'Button Groups',
  'button.large-button': 'Large Button',
  'button.small-button': 'Small Button',
  'button.extra-small-button': 'Extra Small Button',
  'button.block-button': 'Block Button',
  'button.active': '啟用',
  'button.disabled': '草稿',
  'button.basic': 'Basic',
  'button.toolbar': 'Toolbar',
  'button.nesting': 'Nesting',
  'button.vertical-variation': 'Vertical Variation',
  'button.checkbox-radio-button': 'Checkbox and Radio Button',
  'button.checkbox': 'Checkbox',
  'button.radio': 'Radio',
  'button.radio-button': 'Radio Button',
  'button.primary': 'Primary',
  'button.secondary': 'Secondary',
  'button.success': 'Success',
  'button.danger': 'Danger',
  'button.warning': 'Warning',
  'button.info': 'Info',
  'button.light': 'Light',
  'button.dark': 'Dark',
  'button.states-text':
    'This button shows a spinner for 2 seconds and an error icon for 3 seconds before switching to normal state. This states can be triggered by adding and removing classes.',
  'button.click-here': 'Click Here',
  'button.states-text-alternate':
    'This button shows a spinner for 2 seconds and an error icon for 3 seconds before switching to normal state. This states can be triggered by adding and removing classes.',
  'button.primary-link': 'Primary Link',
  'button.link': 'Link',
  'button.primary-button': 'Primary Button',
  'button.button': 'Button',
  'button.left': 'Left',
  'button.middle': 'Middle',
  'button.right': 'Right',
  'button.dropdown': 'Dropdown',
  'button.dropdown-link': 'Dropdown Link',

  /* 07.04.Cards */
  'cards.icon-card': 'Icon Card',
  'cards.image-card': 'Image Card',
  'cards.image-overlay-card': 'Image Overlay Card',
  'cards.image-card-list': 'Image Card List',
  'cards.tab-card': 'Tab Card',
  'cards.user-card': 'User Card',

  /* 07.05.Carousel */
  'carousel.basic': 'Carousel Basic',
  'carousel.single': 'Carousel Single',
  'carousel.without-controls': 'Carousel Without Controls',

  /* 07.06.Charts */
  'charts.line': 'Line Chart',
  'charts.polar': 'Polar Chart',
  'charts.area': 'Area Chart',
  'charts.scatter': 'Scatter Chart',
  'charts.bar': 'Bar Chart',
  'charts.radar': 'Radar Chart',
  'charts.pie': 'Pie Chart',
  'charts.doughnut': 'Doughnut Chart',
  'charts.shadow': 'Shadow',
  'charts.no-shadow': 'No Shadow',

  /* 07.07.Collapse */
  'collapse.basic': 'Basic',
  'collapse.toggle': 'Toggle',
  'collapse.accordion': 'Accordion',
  'collapse.controlled': 'Controlled',
  'collapse.uncontrolled': 'Uncontrolled',

  /* 07.08.Dropdowns */
  'dropdowns.basic': 'Basic',
  'dropdowns.controlled': 'Controlled',
  'dropdowns.uncontrolled': 'Uncontrolled',
  'dropdowns.dropdown': 'Dropdown',
  'dropdowns.header': 'Header',
  'dropdowns.action': 'Action',
  'dropdowns.another-action': 'Another Action',
  'dropdowns.right': 'Right',
  'dropdowns.left': 'Left',
  'dropdowns.drop-directions': 'Drop Directions',
  'dropdowns.dropright': 'Dropright',
  'dropdowns.dropleft': 'Dropleft',
  'dropdowns.small-button': 'Small Button',
  'dropdowns.large-button': 'Large Button',
  'dropdowns.sizing': 'Sizing',
  'dropdowns.split-button': 'Split Button Dropdowns',
  'dropdowns.dropup': 'Dropup',
  'dropdowns.profile': '查看資料',
  'dropdowns.edit': '編輯',
  'dropdowns.delete': '刪除',

  /* 07.09.Editors */
  'editors.draftjs': 'Draft.js',
  'editors.quill-standart': 'Quill Standard',
  'editors.quill-bubble': 'Quill Bubble',

  /* 07.10.Forms */
  'forms.basic': 'Basic',
  'forms.email': 'E-mail',
  'forms.email-muted': '輸入公司名稱',
  'forms.password': 'Password',
  'forms.password-confirm': 'Confirm Password',
  'forms.submit': '確認',
  'forms.horizontal': 'Horizontal',
  'forms.radios': 'Radios',
  'forms.first-radio': 'First radio',
  'forms.second-radio': 'Second radio',
  'forms.third-radio-disabled': 'Third disabled radio',
  'forms.checkbox': 'Checkbox',
  'forms.signin': 'Sign in',
  'forms.top-labels-over-line': 'Top Labels Over Line',
  'forms.tags': 'Tags',
  'forms.date': 'Date',
  'forms.state': 'State',
  'forms.top-labels-in-input': 'Top Labels In Input',
  'forms.email-u': 'E-MAIL',
  'forms.password-u': 'PASSWORD',
  'forms.tags-u': 'TAGS',
  'forms.date-u': 'DATE',
  'forms.state-u': 'STATE',
  'forms.grid': 'Form Grid',
  'forms.address': 'Address',
  'forms.address2': 'Address 2',
  'forms.city': 'City',
  'forms.city-message': 'Please select a city!',
  'forms.state-message': 'Please select a state!',
  'forms.zip': 'Zip',
  'forms.signup': 'Signup',
  'forms.inline': 'Inline',
  'forms.validation-availity': 'Availity Reactstrap Validation',
  'forms.validation-formik': 'Formik Validation',
  'forms.default': 'Default',
  'forms.firstname': '名字',
  'forms.firstname-message': 'Please enter your first name!',
  'forms.lastname': 'Last name',
  'forms.lastname-message': 'Please enter your last name!',
  'forms.name': 'Name',

  /* 07.11.Form Components */
  'form-components.custom-inputs': 'Custom Inputs',
  'form-components.checkboxes': 'Checkboxes',
  'form-components.radios': 'Radios',
  'form-components.inline': 'Inline',
  'form-components.react-select': 'React Select',
  'form-components.state-single': 'State Single',
  'form-components.state-multiple': 'State Multiple',
  'form-components.react-autosuggest': 'React Autosuggest',
  'form-components.date-picker': 'Date Picker',
  'form-components.date': 'Date',
  'form-components.date-range': 'Date Range',
  'form-components.date-with-time': 'Date with Time',
  'form-components.embedded': 'Embedded',
  'form-components.dropzone': 'Dropzone',
  'form-components.drop-files-here': 'Drop Files Here',
  'form-components.tags': 'Tags',
  'form-components.switch': 'Switch',
  'form-components.primary': 'Primary',
  'form-components.secondary': 'Secondary',
  'form-components.primary-inverse': 'Primary Inverse',
  'form-components.secondary-inverse': 'Secondary Inverse',
  'form-components.slider': 'Slider',
  'form-components.double-slider': 'Double Slider',
  'form-components.single-slider': 'Single Slider',
  'form-components.rating': 'Rating',
  'form-components.interactive': 'Interactive',
  'form-components.readonly': 'Readonly',
  'form-components.type-a-cake': 'Type a Cake',
  'form-components.start': 'Start',
  'form-components.end': 'End',
  'form-components.tables': 'Tables',

  /* 07.11.Added by Stephen */
  'menu.users-list': '管理員目錄',
  'menu.users': '管理員',
  'menu.staff-profile': '個人資料',
  'menu.clients-edit': '公司登錄',
  'menu.staff-edit': '編輯員工資料',
  'form-staff-headshot': '員工照片 (建議尺寸: 300px x 300px. 圖像必須是正方形，文件名中不能包含空格和符號)',
  'form-staff-wechat-qrCode': '在此上傳微信二維碼圖片',
  'forms.user-company': '公司名稱',
  'forms.staff-work_email': '工作電郵',
  'forms.staff-home_email': '私人電郵',
  'forms.staff-other_email': '其他電郵',
  'forms.staff-position': '職位',
  'forms.staff-work_tel': '工作電話-1',
  'forms.staff-work_tel2': '工作電話-2',
  'forms.staff-mobile': '手機-1',
  'forms.staff-mobile2': '手機-2',
    'forms.staff-mobile3': '手機-3',
    'forms.staff-mobile4': '手機-4',
  'forms.staff-home_tel': '家居電話',
  'forms.staff-fax': '傳真-1',
  'forms.staff-fax2': '傳真-2',
  'forms.staff-web_link': '網站-1',
  'forms.staff-web_link2': '網站-2',
  'forms.staff-web_link3': '網站-3',
  'forms.staff-web_link4': '網站-4',
  'forms.staff-web_link5': '網站-5',
  'forms.staff-web_link6': '網站-6',
  'forms.staff-address': '地址-1',
  'forms.staff-address2': '地址-2',
  'forms.staff-division': 'Division',
  'forms.staff-department': '部門',
  'forms.staff-country': '國家',
  'forms.staff-bio': 'Bio',
  'forms.staff-company_website_url': '公司網站',
  'forms.staff-more_info_tab_url': '更多信息',
  'forms.staff-facebook_url': 'Facebook 臉書',
  'forms.staff-instagram_url': 'Instagram',
  'forms.staff-twitter_url': 'Twitter 推特',
  'forms.staff-whatsapp_url': 'Whatsapp',
  'forms.staff-linkedin_url': 'Linkedin 领英',
  'forms.staff-youtube_url': 'Youtube',
  'forms.staff-tiktok_url': 'TikTok',
  'forms.staff-line_url': 'Line',
  'forms.staff-facebook_messenger_url': 'Facebook臉書 Messenger',
  'forms.staff-weibo_url': '微博',
  'forms.staff-qq_url': 'QQ',
  'forms.staff-zhihu_url': '知乎',
  'forms.staff-appsstore_url': 'Apple Apps Store',
  'forms.staff-googleplay_url': 'Google Play Store',
  'forms.staff-location_url': '位置',
  'forms.staff-video_url': '視頻',
  'forms.staff-telegram_url': 'Telegram',
  'forms.staff-wechat_id': '微信ID',
  'forms.staff-wechat_qr_url': '微信 QR Url',
  'forms.staff-smartcard_uid': '智能名片ID',
  'forms.staff-wechat_qr_url-muted': 'e.g. https://u.wechat.com/ABCDEXXXXXFFF',
 'forms.staff-work_email-muted': 'e.g. abc@gmail.com - 在e-Profile和vcf和Vcard出現',
  'forms.staff-home_email-muted': 'e.g. abc@gmail.com - 在e-Profile和vcf出現',
  'forms.staff-other_email-muted': 'e.g. abc@gmail.com - 在e-Profile和vcf出現',
  'forms.staff-position-muted': 'e.g. Sales Manager - 在e-Profile和vcf和Vcard出現',
  'forms.staff-work_tel-muted': 'e.g. +852 xxxx xxxx - 在e-Profile和vcf和Vcard出現',
  'forms.staff-work_tel2-muted': 'e.g. +852 xxxx xxxx - 在e-Profile和vcf出現',
  'forms.staff-mobile-muted': 'e.g. +852 6xxx xxxx - 在e-Profile和vcf和Vcard出現',
  'forms.staff-mobile2-muted': 'e.g. +852 6xxx xxxx - 在e-Profile和vcf出現',
    'forms.staff-mobile3-muted': 'e.g. +852 6xxx xxxx - 在e-Profile和vcf出現',
    'forms.staff-mobile4-muted': 'e.g. +852 6xxx xxxx - 在e-Profile和vcf出現',
  'forms.staff-home_tel-muted': 'e.g. +852 xxxx xxxx - 在e-Profile和vcf出現',
  'forms.staff-fax-muted': 'e.g. +852 xxxx xxxx - 在e-Profile和vcf出現',
  'forms.staff-fax2-muted': 'e.g. +852 xxxx xxxx - 只在vcf出現',
  'forms.staff-web_link-muted': 'e.g. https://xxx.com - 在e-Profile和vcf出現',
  'forms.staff-web_link2-muted': 'e.g. https://xxx.com - 在e-Profile和vcf出現',
  'forms.staff-web_link3-muted': 'e.g. https://xxx.com - 在e-Profile和vcf出現',
  'forms.staff-web_link4-muted': 'e.g. https://xxx.com - 在e-Profile和vcf出現',
  'forms.staff-web_link5-muted': 'e.g. https://xxx.com - 在e-Profile和vcf出現',
  'forms.staff-web_link6-muted': 'e.g. https://xxx.com - 在e-Profile和vcf出現',
  'forms.staff-address-muted': 'e.g. xxx, xx street, Country - 在e-Profile和vcf和Vcard出現',
  'forms.staff-address2-muted': 'e.g. xxx, xx street, Country - 在e-Profile和vcf出現',
  'forms.staff-division-muted': 'e.g. XXX Group',
  'forms.staff-department-muted': 'e.g. 行政部',
  'forms.staff-country-muted': 'e.g. 國家',
  'forms.staff-bio-muted': 'e.g. 傳記或執照 - 只在e-Profile出現',
'forms.staff-company_website_url-muted': 'e.g. https://xxx.com - 在e-Profile和vcf和Vcard出現',
  'forms.staff-more_info_tab_url-muted': 'e.g. https://xxx.com - 在e-Profile和vcf出現',
  'forms.staff-facebook_url-muted': 'e.g. https://www.facebook.com/xxx - 在e-Profile和vcf出現',
  'forms.staff-instagram_url-muted': 'e.g. https://www.instagram.com/ - 在e-Profile和vcf出現',
  'forms.staff-twitter_url-muted': 'e.g. https://twitter.com/xxx - 在e-Profile和vcf出現',
  'forms.staff-whatsapp_url-muted': 'e.g. https://api.whatsapp.com/send/?phone=xxxxxxxx - 在e-Profile和vcf出現',
  'forms.staff-linkedin_url-muted': 'e.g. https://www.linkedin.com/xxx - 在e-Profile和vcf出現',
  'forms.staff-youtube_url-muted': 'e.g. https://www.youtube.com/xxx - 在e-Profile和vcf出現',
  'forms.staff-tiktok_url-muted': 'e.g. https://www.tiktok.com/xxx - 在e-Profile和vcf出現',
  'forms.staff-line_url-muted': 'e.g. https://line.me/xxx - 在e-Profile和vcf出現',
  'forms.staff-facebook_messenger_url-muted': 'e.g. https://www.messenger.com/xxx - 在e-Profile和vcf出現',
  'forms.staff-weibo_url-muted': 'e.g. https://s.weibo.com/xxx - 在e-Profile和vcf出現',
  'forms.staff-qq_url-muted': 'e.g. https://im.qq.com/xxx - 在e-Profile和vcf出現',
  'forms.staff-zhihu_url-muted': 'e.g. https://www.zhihu.com/xxx - 在e-Profile和vcf出現',
  'forms.staff-appsstore_url-muted': 'e.g. https://apps.apple.com/xxx - 在e-Profile和vcf出現',
  'forms.staff-googleplay_url-muted': 'e.g. https://play.google.com/store/apps/xxx - 在e-Profile和vcf出現',
  'forms.staff-location_url-muted': 'e.g. https://www.google.com.hk/maps/xxx - 在e-Profile和vcf出現',
  'forms.staff-video_url-muted': 'e.g. https://www.youtube.com/xxx - 在e-Profile和vcf出現',
  'forms.staff-telegram_url-muted': 'e.g. https://web.telegram.org/xxx - 在e-Profile和vcf出現',
  'forms.staff-wechat_id-muted': 'e.g. XXXXXXX - 只在e-Profile出現',
  'forms.staff-smartcard_uid-muted': 'e.g. 12345678PIMI2',
  'forms.staff-status': '員工狀態',
  'menu.smartcard-list': '智能卡片',
  'forms.staff-bizcard_option': '智能卡選項',
  'menu.hr.users': '註冊管理員',
  'menu.staffs.profile': '員工目錄',
  'menu.hr.admin': '管理員',
  'menu.user-profile': '個人資料設置',
  'menu.card-edit': '編輯',
  'menu.staffs-profile': '員工資料',
  'pages.address': '地址',
  'pages.position': '職稱',
  'pages.Website': '網站',
  'pages.work_tel': '電話',
  'pages.mobile': '手機',
  'pages.work_email': '電子郵件',
  'pages.additionalInfo': '電子名片',
  'menu.webprofilepage': '數碼檔案',
  'menu.downloadVCFContent': '下載聯繫資料',
  'menu.socialMedia': '社交媒體',
 
  'menu.client-profile': '編輯',
  'forms.client-name': '客戶名稱',
  'forms.client-code': '客戶代碼',
  'forms.client-no_of_license': '註冊員工數量',
  'forms.client-no_of_admin': '管理員數量',
  'forms.client-status': '客戶狀態',
  'menu.clients-add': '客戶賬戶',
  'menu.cards-list': '清單',
  'menu.staffs-list': '資料清單',
  'menu.smartcard.list': '智能卡片',
  'menu.smartcards-list': '智能卡片清單',
  'menu.admin-edit': '管理員設置',
  'menu.admins': '管理員密碼',
  'input-groups.update-password': '更新管理員密碼',
  'forms.admin-update-password': '輸入新密碼',
  'forms.admin-re-type-update-password': '重新輸入新密碼',
  'input-groups.admin-staff-profile': '管理員',
  'form-company-banner': '封面照片 (建議尺寸: 2000px x 1052px. 圖像必須是横向的，文件名中不能包含空格和符號)',
  'form-company-logo': '公司商標 (建議尺寸: 300px x 300px. 圖像必須是正方形，文件名中不能包含空格和符號)',
  'forms.company-name': '公司名稱',
  'forms.staff-company_name-muted': '例如: ABC Company Ltd',
  'forms.company-country': '國家',
  'forms.admin-company-country-muted': '例如: 香港',
  'forms.company-website': '公司網站',
  'forms.company-website-muted': '例如: www.abc.com',
  'forms.company-position': '職位',
  'forms.company-position-muted': '例如: Admin Manager',
  'forms.company-work-tel': '電話',
  'forms.company-work_tel-muted': '例如: +852 3409 0293',
  'forms.company-mobile': '手機',
  'forms.company-mobile-muted': '例如: +852 9409 0293',
  'forms.admin-company-address': '地址',
  'forms.admin-company-address-muted': '例如: 123 King Road',
  'forms.company-sub-division': '集團',
  'forms.company-sub_division-muted': '例如: Business Unit - Hong Kong',
  'forms.company-department': '部門',
  'forms.company-department-muted': '例如: Central Admin',

  'profile.label.position': '職位',
  'profile.button.edit': '編輯個人資料',
  'profile.label.division': '集團',
  'profile.label.country': '國家',
  'pages.department': '部門',
    'pages.staff_no': '職員編號',
  'profile.button.moreInfo': '瀏覽',
  'profile.button.goProfile': '瀏覽',
  'profile.button.downloadVcf': '瀏覽',
  'pages.QRcode': '二維碼',
  'dashboards.profile-visits': '電子名片',
  'dropdowns.admin.edit': '管理員設置',
  'forms.smartcard-uid': '智能卡 UID',
  'forms.smartcard-muted': '例如: 8U0I92INP012N83G',
  'forms.smartcards-status': '狀態',
  'menu.cards-add': '智能卡管理',
  'menu.user-edit': '編輯管理員',
  'forms.user-username': '用戶名',
  'forms.user-username-muted': 'ABC_Company',
  'forms.user-email': '電郵',
  'forms.user-email-muted': 'e.g. admin@abc.com',
  'forms.user-password': '密碼',
  'forms.user-status': '狀態',
  'menu.client-edit': '公司註冊資料',  
'menu.admin-list-header': '管理員註冊',
  'menu.staffs-list-header': '員工列表',
     'menu.users-add':'添加員工',
    'menu.staff-add':'添加',
            'menu.staff-edit-title':'編輯員工資料',
            'forms.label.eprofile': 'e-Profile ',
            'forms.label.vcf': 'vcf ',
        'menu.smartcard.list.admin': '智能卡',
    'menu.cardsadmin': '編輯智能卡',
            'menu.batchupload': '批量上傳',
            'menu.batch.staff.upload': '批量上傳員工資料',
    'menu.staffbatchupload': '批量上傳',
    'menu.': '設置',
        'form-staff-excel-upload': '上傳Excel',
            'batchupload.download.staff.excel': '導出Excel',
            'forms.hr-work_email-muted': 'e.g. abc@gmail.com',
        'forms.admin-password-muted': 'e.g. 12345abcd',
                    'dashboards.this-daily-or-monthly': '統計',
        'dashboards.this-daily': '按日',
        'dashboards.this-monthly': '按月',
    
     'forms.staff-fname': '英文名稱',
    'forms.staff-firstname-muted': 'E.g. John',
    'forms.staff-lastname': '英文姓氏',
    'forms.staff-lastname-muted': 'E.g. Smith/ Chan',   
    'forms.staff-company_name_eng': '公司名稱',
    'forms.staff-company_name_eng-muted': 'E.g. ABC Limited',
    'forms.staff-company_name_chi': '公司名稱(其他語言)',
    'forms.staff-company_name_chi-muted': 'E.g. 一二三有限公司', 
    'forms.staff-staff_no': '員工編號',
  'forms.staff-work_email2': '工作電郵 2',
    'forms.staff-work_email2-muted':'e.g. abc@gmail.com - 在e-Profile和vcf出現',
    'forms.staff-work_email3': '工作電郵 3',
    'forms.staff-work_email3-muted':'e.g. abc@gmail.com - 在e-Profile和vcf出現',
    'forms.staff-work_tel3':'工作電話-3',
        'forms.staff-work_tel3-muted':'e.g. abc@gmail.com - 在e-Profile和vcf出現',
        'forms.staff-work_tel4':'工作電話-4',
        'forms.staff-work_tel4-muted':'e.g. abc@gmail.com - 在e-Profile和vcf出現',
        'forms.staff-web_link_label': '網站-1 標題',
        'forms.staff-web_link_label-muted': 'E.g. Brochure',
            'forms.staff-web_link_label2': '網站-2 標題',
        'forms.staff-web_link_label2-muted': 'E.g. Questionnaire',
            'forms.staff-web_link_label3': '網站-3 標題',
        'forms.staff-web_link_label3-muted': 'E.g. Our Location',
            'forms.staff-web_link_label4': '網站-4 標題',
        'forms.staff-web_link_label4-muted': 'E.g. Payment',
                'forms.staff-web_link_label5': '網站-5 標題',
        'forms.staff-web_link_label5-muted': 'E.g. E-shop',
            'forms.staff-web_link_label6': '網站-6 標題',
        'forms.staff-web_link_label6-muted': 'E.g. Order Form',
    'forms.staff-address3': '地址-3',
    'forms.staff-address3-muted': 'e.g. xxx, xx street, Country - 在e-Profile和vcf出現',
        'forms.staff-address4': '地址-4',
    'forms.staff-address4-muted': 'e.g. xxx, xx street, Country - 在e-Profile和vcf出現',
        'forms.staff-wechatpage_url': '微信',
        'forms.staff-wechatpage_url-muted': 'e.g. https://www.wechat.com/xxx - 在e-Profile和vcf出現',
    'forms.staff-douyin_url': '抖音',
        'forms.staff-douyin_url-muted': 'e.g. https://www.douyin.com/xxx - 在e-Profile和vcf出現',
            'forms.staff-kuaishou_url': '快手',
        'forms.staff-kuaishou_url-muted': 'e.g. https://www.kuaishou.com/xxx - 在e-Profile和vcf出現',
        'forms.staff-bilibili_url': '哔哩哔哩',
                'forms.staff-bilibili_url-muted': 'e.g. https://www.bilibili.com/xxx - 在e-Profile和vcf出現',
        'forms.staff-app_store_url': '蘋果 Apps Store',
        'forms.staff-app_store_url-muted': 'e.g. https://apps.apple.com/xxxx - 在e-Profile和vcf出現',
    'forms.staff-google_play_url': 'Google Play 商店',
        'forms.staff-google_play_url-muted': 'e.g. https://play.google.com/xxx - 在e-Profile和vcf出現',
            'forms.staff-snapchat_url': 'Snapchat',
        'forms.staff-snapchat_url-muted': 'e.g. https://www.snapchat.com/xxx - 在e-Profile和vcf出現',
        'forms.staff-xiaohongshu_url': '小紅書',
                'forms.staff-xiaohongshu_url-muted': 'e.g. https://www.xiaohongshu.com/xxx - 在e-Profile和vcf出現',
        'forms.staff-note': '注',
        'forms.staff-note-muted': 'e.g. XXX牌照號碼: 1234 - 只在vcf出現',
'forms.staff-qrcode_option': 'e-Profile上的二維碼',
'cards.tab-admin-qr1': 'Vcard',
'cards.tab-admin-qr2': '智能卡同步',
'cards.tab-admin-qr3': 'E-Profile網頁',
'cards.tab-admin-qr4': 'VCF',
'input-groups.update-2fa': '2FA 設置',
'forms.admin-email': '管理員電郵',
'forms.admin-email-muted': 'e.g. 此電子郵件將收到6位數的驗證碼',
'forms.admin-two-factor': '2FA 功能',
'cards.tab-admin-log1': '人員資料變更記錄',
'cards.tab-admin-log2': 'E-Profile',
'cards.tab-admin-log3': 'VCF',
'forms.download_staff_log_excel': '下載',
'forms.download_staff_profile_excel': '下載',
'forms.download_staff_vcf_excel': '下載',
'form-company-profile-theme': 'E-Profile背景圖 (建議尺寸: 1052px x 2000px. 圖像必須是垂直的，文件名中不能包含空格和符號)',
   'forms.staff-note-timestamp': '備注記錄見面時間',
             'user.verify-title': '2FA認證',
        'user.tokeninput': '6位數驗證碼',
        'user.back-to-login': '返回',
            'forms.label.note-timestamp-on': '開啟',
        'forms.label.note-timestamp-off': '關掉',
    	'forms.staff-dig_card_in_vcf-option': '下載聯繫人中包含電子名片',
	'forms.label.enable': '啟用',
	'forms.label.disable': '關閉',
        'forms.staff-mname':'中間名',
    'forms.staff-middlename-muted': 'E.g. Simpson/ Tai Man',
    'forms.staff-prefixname': '字首',
    'forms.staff-prefixname-muted': 'E.g. Dr., Sir, Captain',
    'forms.staff-othername': '其他名字',
    'forms.staff-othername-muted': 'E.g. 陳大文',
    'forms.staff-prodesname': '專業頭銜',
    'forms.staff-prodesname-muted': 'E.g. CFA',
    'dropdowns.deactive': '停用',
    'menu.staffsdactive.profile': '停用人員',
        'menu.staffsdeactive': '停用人員',
    'dropdowns.active': '激活',
    'menu.card-add':'添加卡',
    'menu.staffs-list-deactive-header':'已停用人員列表',
        'forms.staff-minisite_option':'電子卡選項',
    'menu.user-add':'新增管理員用戶',
    'cards.tab-admin-log7':'完整員工名單',
    'forms.download_staff_link_excel':'下載',
     'menu.hr.wallet':'電子錢包',
    'menu.wallets':'錢包選項',
    'menu.wallet-edit':'電子錢包名片',
    'input-groups.admin-wallet-setting':'客製化設計',
    'form-company-wallet-banner':'電子錢包橫幅 (建議尺寸: 1032px x 336px. 圖像必須是横向的，文件名中不能包含空格和符號)',
    'forms.staff-wallet_logo_option':'圖像',
    'forms.staff-wallet_field1_option':'標題 1',
    'forms.staff-wallet_field2_option':'文字 2',
    'forms.staff-wallet_field3_option':'文字 3',
    'forms.staff-wallet_qrcode_option':'選擇二維碼',
    'forms.staff-wallet_text_color':'文字顏色',
    'forms.staff-wallet_text_color-muted':'e.g. #FFFFFF',
    'forms.staff-wallet_bg_color':'背景顏色',
    'forms.staff-wallet_bg_color-muted':'e.g. #000000',
    'cards.tab-admin-qr5':'Google錢包名片',
    'cards.tab-admin-qr6':'Apple錢包名片',
    'cards.tab-admin-qr7':'網頁版名片',
    'cards.tab-admin-qr8':'電子錢包名片',
    'cards.tab-admin-log5':'Apple錢包名片',
    'cards.tab-admin-log6':'網頁版名片',
    'cards.tab-admin-log4':'Google錢包名片',
    'forms.download_staff_gw_excel':'下載',
    'forms.download_staff_aw_excel':'下載',
    'forms.download_staff_mobilesite_excel':'下載',
        'forms.staff-work_email_label-muted':'預設: Email',
    'forms.staff-work_email2_label-muted':'預設: Email',
    'forms.staff-work_email3_label-muted':'預設: Email',
    'forms.staff-home_email_label-muted':'預設: Home Email',
    'forms.staff-other_email_label-muted':'預設: Other Email',
    'forms.staff-work_tel_label-muted':'預設: Tel',
    'forms.staff-work_tel2_label-muted':'預設: Tel',
    'forms.staff-work_tel3_label-muted':'預設: Tel',
    'forms.staff-work_tel4_label-muted':'預設: Tel',
    'forms.staff-mobile_label-muted':'預設: Mobile',
    'forms.staff-mobile2_label-muted':'預設: Mobile',
    'forms.staff-mobile3_label-muted':'預設: Mobile',
    'forms.staff-mobile4_label-muted':'預設: Mobile',
    'forms.staff-home_tel_label-muted':'預設: Home Tel',
    'forms.staff-fax_label-muted':'預設: Fax no.',
    'forms.staff-address_label-muted':'預設: Address',
    'forms.staff-address2_label-muted':'預設: Address',
    'forms.staff-address3_label-muted':'預設: Address',
    'forms.staff-address4_label-muted':'預設: Address',
        'forms.staff-wallet_field2_title': '文字位置2 - 自訂標題',
  'forms.staff-wallet_field3_title': '文字位置3 - 自訂標題',
     'forms.staff-position_other_lang-muted': 'e.g. 銷售經理 - 在e-Profile和vcf和Vcard出現',
    'forms.staff-awards': '獎項',
  'forms.staff-awards-muted': 'e.g. 2024 - 金牌資訊安全獎',
  'forms.staff-qualifications': '資格',
  'forms.staff-qualifications-muted': 'e.g. 醫學博士 (MD) - ABC 大學',
  'forms.staff-additional_address': '辦公地點',
  'forms.staff-additional_address-muted': 'e.g. 分行 1：香港ABC道123號',
  'forms.staff-achievements': '成就',
  'forms.staff-achievements-muted': 'e.g. 藝術開發技術顧問',
    
    
  /* 07.12.Icons */
  'icons.simplelineicons': 'Simple Line Icons',
  'icons.iconsmind': 'Iconsmind Icons',

  /* 07.13.Input Groups */
  'input-groups.basic': 'Basic',
  'input-groups.sizing': 'Sizing',
  'input-groups.small': 'Small',
  'input-groups.default': 'Default',
  'input-groups.large': 'Large',
  'input-groups.checkboxes-and-radios': 'Checkboxes and radios',
  'input-groups.multiple-inputs': '姓名',
  'input-groups.first-and-last-name': 'First and last name',
  'input-groups.multiple-addons': 'Multiple Addons',
  'input-groups.button-addons': 'Button Addons',
  'input-groups.button': 'Button',
  'input-groups.buttons-with-dropdowns': 'Buttons with Dropdowns',
  'input-groups.dropdown': 'Dropdown',
  'input-groups.header': 'Header',
  'input-groups.action': 'Action',
  'input-groups.another-action': 'Another Action',
  'input-groups.custom-select': 'Custom Select',
  'input-groups.options': 'Options',
  'input-groups.choose': 'Choose...',
  'input-groups.custom-file-input': 'Custom File Input',

  /* 07.14.Jumbotron */
  'jumbotron.hello-world': 'Hello, world!',
  'jumbotron.lead':
    'This is a simple hero unit, a simple jumbotron-style component for calling extra attention to featured content or information.',
  'jumbotron.lead-detail':
    'It uses utility classes for typography and spacing to space content out within the larger container.',
  'jumbotron.learn-more': 'Learn more',

  /* 07.15.Modal */
  'modal.basic': 'Basic',
  'modal.modal-title': 'Modal title',
  'modal.launch-demo-modal': 'Launch Demo Modal',
  'modal.scrolling-long-content': 'Scrolling Long Content',
  'modal.backdrop': 'Backdrop',
  'modal.backdrop-value': 'Backdrop value',
  'modal.right-modal': 'Right Modal',
  'modal.launch-right-modal': 'Launch Right Modal',
  'modal.nested-modal': 'Nested Modal',
  'modal.size': 'Size',
  'modal.launch-large-modal': 'Launch Large Modal',
  'modal.launch-small-modal': 'Launch Small Modal',

  /* 07.16.Navigation */
  'nav.basic': 'Nav Basic',
  'nav.active': '啟用',
  'nav.disabled': '草稿',
  'nav.disabled-link': 'Disabled Link',
  'nav.link': 'Link',
  'nav.longer-link': 'Longer nav link',
  'nav.another-link': 'Another Link',
  'nav.right': 'Right',
  'nav.dropdown': 'Dropdown',
  'nav.header': 'Header',
  'nav.action': 'Action',
  'nav.another-action': 'Another Action',
  'nav.horizontal-alignment': 'Nav Horizontal Alignment',
  'nav.vertical-alignment': 'Nav Vertical Alignment',
  'nav.pills': 'Nav Pills',
  'nav.fill-justify': 'Nav Fill and Justify',
  'nav.pills-dropdowns': 'Nav Pills with Dropdowns',
  'nav.pagination-basic': 'Pagination Basic',
  'nav.pagination-sizing': 'Pagination Sizing',
  'nav.large': 'Large',
  'nav.small': 'Small',
  'nav.center': 'Center',
  'nav.pagination-alignment': 'Pagination Alignment',
  'nav.breadcrumb-basic': 'Breadcrumb Basic',

  /* 07.17.Popover & Tooltip */
  'popover-tooltip.popover': 'Popover',
  'popover-tooltip.tooltip': 'Tooltip',

  /* 07.18.Sortable */
  'sortable.columns': 'Sorting Columns',
  'sortable.basic': 'Basic',
  'sortable.handles': 'Handles',

  /* 07.19.Maps */
  'maps.google': 'Google',
  'maps.yandex': 'Yandex',

  /* 07.20.Tables */
  'table.bootstrap-tables': 'Bootstrap Tables',
  'table.bootstrap-basic': 'Basic Table',
  'table.bootstrap-striped': 'Striped Rows',
  'table.bootstrap-bordered': 'Bordered Table',
  'table.bootstrap-borderless': 'Borderless Table',
  'table.bootstrap-hoverable': 'Hoverable Rows',
  'table.bootstrap-responsive': 'Responsive Table',
  'table.react-tables': 'React Tables',
  'table.react-pagination': 'Pagination',
  'table.react-scrollable': 'Scrollable',
  'table.react-advanced': 'Filter, Length and Jump',
  'table.divided': 'Divided Table',

  /* 07.21.Wizards */
  'wizard.step-name-1': 'Step 1',
  'wizard.step-name-2': 'Step 2',
  'wizard.step-name-3': 'Step 3',
  'wizard.step-desc-1': 'First step description',
  'wizard.step-desc-2': 'Second step description',
  'wizard.step-desc-3': 'Third step description',
  'wizard.content-1': 'Step content for first step.',
  'wizard.content-2': 'Step content for second step.',
  'wizard.content-3': 'Last step content!',
  'wizard.content-thanks': 'Thank You!',
  'wizard.next': 'Next',
  'wizard.prev': 'Back',
  'wizard.registered': 'Your registration completed successfully!',
  'wizard.async': 'Async save for 3 seconds!',

  /* 2024/12/26 Minisite */
   'menu.style.minisite': 'E-Profile頁面',
  'menu.style.minisite.admin': 'E-Profile頁面',
  'menu.minisite-list': 'E-Profile頁面',
  'menu.minisite': 'E-Profile頁面',
  'menu.minisite-edit': '定制化',
  'forms.minisite-fontcolor': '字體顏色',
  'forms.minisite-fontcolor-muted': 'E.g #FFFFFF',
  'forms.minisite-fontsize': '文字大小',
  'forms.minisite-fontsize-muted': '10',
  'forms.minisite-fontfamily': '字體樣式',
  'forms.minisite-fontfamily-muted': 'Arial Black',
  'forms.minisite-text-color': '其他文字顏色',
  'forms.minisite-title-text-color': '次要主題顏色',
  'forms.minisite-social-icon-bg-color': '社交圖示顏色',
  'forms.minisite-button-color': '按鈕顏色',
  'forms.minisite-links-hover-color': '側邊欄 - 懸停按鈕顏色',
  'forms.minisite-links-not-hover-color': '側邊欄 - 非活動頁面顏色',
  'forms.minisite-key-wording-color': '主要主題顏色',
  'forms.minisite-site-bg-color': '頁面顏色',
  'forms.minisite-left-nav-bar-color': '側邊欄 - 顏色',
  'forms.minisite-bio-wording-color': '個人簡介 (Bio) 文字顏色',
  'forms.minisite-backgroundcolor': '背景顏色',
  'forms.minisite-backgroundcolor-muted': 'E.g #FFFFFF',
  'forms.minisite-backgroundimg': 'Background Image (Recommended Size: 1920px x 1134px. Must be in square and do not include space and symbol in the filename)',
  'forms.minisite-backgroundimg-muted': 'https',
  'forms.minisite-links-selected-color':'側邊欄 - 活動頁面顏色',
  'forms.crop.ok':'確認',
  'forms.crop.cancel':'取消',
          'forms.title-font-size':'主題文字大小',
  'forms.title-font-size-muted':'16',
    
      /* 2025/11/06 Admin Page */
  'forms.admin-logo_display_option':'E-Profile上的公司標誌',
  'forms.admin-headshot_display_option':'E-Profile上的頭像',
};
